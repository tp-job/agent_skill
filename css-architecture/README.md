...
### For a New Project
1. Read: [SKILL.md](./SKILL.md) Step 0 + Golden Rule
2. Choose: Small/Medium/Large from [folder-templates](references/folder-templates.md)
3. Copy: `main.css.template` → `src/styles/main.css`
4. Create: `tokens/_root.css` with starter tokens
5. Install: Stylelint config from [linting-setup](references/linting-setup.md)

### For an Existing Project
1. Audit: Run checklist from [migration-guide](references/migration-guide.md)
...
### In Code Review
Use the `QA Validation Checklist` from SKILL.md when reviewing CSS PRs. Link to specific anti-patterns in [anti-patterns](references/anti-patterns.md) if violations occur.

### In Team Onboarding
Point new developers to:
1. [SKILL.md](./SKILL.md) — Golden Rule + Folder Structure sections
2. [anti-patterns](references/anti-patterns.md) — What not to do
...
- CSS conflicts? → [layer-system](references/layer-system.md)
- Folder structure? → [folder-templates](references/folder-templates.md)
- Setting up Stylelint? → [linting-setup](references/linting-setup.md)
- Migrating existing CSS? → [migration-guide](references/migration-guide.md)
- What went wrong? → [anti-patterns](references/anti-patterns.md)
- How to decide? → [SKILL.md](./SKILL.md) Workflow section
