...
- [ ] Copy `.stylelintrc.json` from [linting-setup](./linting-setup.md)
...
Follow the 3-phase plan in [migration-guide](./migration-guide.md):
...
| Stylelint errors on valid `@layer` rules | Update Stylelint config — see [linting-setup](./linting-setup.md) |
| Tailwind utilities not working alongside custom CSS | Missing layer order declaration at top of `main.css` |
| `!important` doesn't override Tailwind utility | Use `@layer overrides` instead — see [layer-system](./layer-system.md) |
...
1. Read: [SKILL.md](../SKILL.md) (sections: Golden Rule, Folder Structure, Workflow)
2. Reference: Keep [anti-patterns](./anti-patterns.md) open during PR review
3. Check: Use QA checklist before submitting CSS PRs
4. Questions: Link to relevant reference file (e.g., "!important issue? See [anti-patterns](./anti-patterns.md) #1")
...
| Need | File | Sections |
|---|---|---|
| Start a new project | [folder-templates](./folder-templates.md) | Small/Medium/Large variants |
| Migrate existing CSS | [migration-guide](./migration-guide.md) | Phase 1/2/3, audit checklist |
| Understand @layer | [layer-system](./layer-system.md) | Why @layer, v3/v4 integration |
| Prevent CSS hell | [anti-patterns](./anti-patterns.md) | 10 patterns, smell test |
| Set up linting | [linting-setup](./linting-setup.md) | Stylelint config, CI setup |
| Main skill reference | [SKILL.md](../SKILL.md) | All sections, decision tree |
