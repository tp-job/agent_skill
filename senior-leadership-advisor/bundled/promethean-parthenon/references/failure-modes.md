...
Each entry: the tell, the real cause, and the pillar that fixes it.
...
**Fix:** **Foresight.** Answer Q1–Q4 and convert every adjective in Q3 into a number, a boundary, or a prohibition.
...
**Fix:** **Structure.** Never claim a pass you did not observe. Unit tests passing ≠ the feature working; verify through the interface the consumer actually touches.
...
**Fix:** **Structure.** `build-spec.md`, `feature-list.json`, `progress.md`, `init.sh` at the repo root. Cold sessions run the full bootstrap.
...
**Fix:** **Structure.** The ledger is append-mostly — only `passes` and `notes` change. A wrong target gets *superseded* in the open, never edited quietly.
...
**Fix:** **Structure**, Stage 2. Grep for the domain noun before writing a new module. Counter-rule: two similar things are not a pattern; wait for three.
...
**Fix:** **Structure**, Stage 2 data mapping. Write the shape on both sides of every boundary and name where conversion happens — exactly once.
...
**Fix:** **Foresight** (a prohibition: "never returns `email`, `passwordHash`, `internalRef`") plus **Structure** (data mapping names what gets dropped).
...
**Fix:** **Structure**, Stage 2 algorithm design — name the per-record loop and state what happens at 100k.
...
**Fix:** **Judgment.** Name the call, the trade-off, and what it makes hard in six months.
...
**Cause:** Structure asks *is this built correctly*, never *should this be built*. It is designed not to ask.

**Fix:** **Judgment**, at the gates and before anything irreversible.
...
**Fix:** **Structure.** Stop after three implement→verify cycles. Re-run decomposition and data mapping before a fourth attempt — the defect is upstream of where you are looking.
...
**Fix:** **Record.** Conventions first, report second — and generate from what is actually there rather than inventing structure the data does not contain.
